...
# data_files = glob.glob('testcase/DNN/*')
# point = []
# X = []
# Y = []
# machine = []
...
# file_path = "testcase/DNN/2022-9-27_10_6_1.json"
# map_path = "map.json"
broken_point_x = []
broken_point_y = []
map_X = []
...
#     json_path = f'testcase/{file_name}'
...
#         total_plt = [merge()]
#     else:
#         all_r.append([total_plt[0][0], total_plt[0][1]])
#     c += 1
# plt.title("all")
# for r in all_r:
...
#########################
# t_p = total_plt[0]
# v = [0 for i in range(len(t_p[0]))]
# v[0] = 1
# r_x = [t_p[0][0]]
# r_y = [t_p[1][0]]
# while len(r_x) != len(t_p[0]):
#     for k in range(1, len(t_p[0])):
#         len1 = round(math.sqrt(math.pow(t_p[0][k] - r_x[-1], 2) + math.pow(t_p[1][k] - r_y[-1], 2)))
#         if len1 <= 8 and v[k] == 0:
#             r_x.append(t_p[0][k])
#             r_y.append(t_p[1][k])
#             v[k] = 1
...
for i in range(0, len(broken_point_x) - 1, 2):
    ...
    if round(x2 - x1) == 0 and round(y2 - y1) == 0:
        continue
    for j in range(len(map_X)):
        if abs(x1 - map_X[j]) < 10 and abs(y1 - map_Y[j]) < 10:
            check_bool = True
            break
        elif abs(x2 - map_X[j]) < 10 and abs(y2 - map_Y[j]) < 10:
            check_bool = True
            break
    if check_bool:
        continue
    gap = math.sqrt(math.pow(x2 - x1, 2) + math.pow(y2 - y1, 2))
    rad = math.atan(40 / (gap / 2))
...
# for i in range(len(total_plt[0][0])):
#     map_X.append(total_plt[0][0][i])
# for i in range(len(total_plt[0][1])):
#     map_Y.append(total_plt[0][1][i])
for i in range(len(total_plt[0][0])):
    map_X.append(total_plt[0][0][i])
for i in range(len(total_plt[0][1])):
    map_Y.append(total_plt[0][1][i])
...
#     plt.scatter(map_X[i], map_Y[i], c="blue", s=5)
# for i in range(len(st_x)):
#     plt.scatter(st_x[i], st_y[i], c="red", s=5)
# plt.show()

file_data["Map_x"] = map_X
file_data["Map_y"] = map_Y
file_data["st_x"] = st_x
file_data["st_y"] = st_y
file_data["total_v"] = total_v
file_data["Location"] = machine
...
# file_data = OrderedDict()
# file_data["Map_x"] = t_r[0]
# file_data["Map_y"] = t_r[1]
# file_path = "./html/index/final.json"
